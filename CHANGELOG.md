...
- `poc/`: a throwaway **Phase 8 experience PoC** that tests whether a small
  on-device LLM can be the arcade's **manager** — run `poc/run.sh` on the Mac,
  pick a scenario, and talk to the cabinet by voice. A **single** container
  mirrors the cabinet's Pi 5 (8GB): Debian 12 Bookworm arm64 (what Raspberry Pi
  OS is), capped at **4 cores / 8GB shared**, running the whole pipeline in one
  box — **faster-whisper** STT (biased to the real names), **Ollama** (`qwen2.5:3b`
  default, kept resident), **Piper** TTS. Each turn is split into small atomic
  steps so a 3B can handle it: a JSON **intent** call, then **code** executes the
  action and **owns all access control** (who is present and what they may do is
  decided in code, never by the model — e.g. only an admin present can set a
  privacy schedule), then a short **phrasing** call (simple replies are canned, no
  model call). The greeting skips the intent call. The actions — `get_player`,
  `list_games`, `recommend_game`, `launch_game`/`close_game`, `assign_joystick`,
  `create_profile`/`delete_profile`, `remember`, `get_context`,
  `set_privacy_schedule` — **enforce truth** (`launch_game` fuzzy-matches a
  misheard "Point" back to "Pong"; unknown games are refused) and **every one is
  printed in the CLI**. Profiles, learned per-person memory ("only plays after
  ...
  functions. Models fetch on first run into cached volumes; the MacBook's mic +
  speakers stand in for the Pi's speakerphone. Key findings, documented: a 3B is
  useless as a one-big-prompt many-tools agent but works well **split into atomic
  steps**; latency lands at **~3–5 s/turn on the Pi-accurate 4-core cap** once
  `num_thread` is matched to the core count (unpinned, Ollama fights the CPU
  throttle, ~4× slower) and the model is kept resident. Caveat: matches the Pi's
  core count/RAM/OS/arch but not per-core clock speed (real perf is Phase 8.9).
  Deterministic core (intent execution + access control, tools, store,
  privacy-window logic, analytics) is unit-tested with no model/audio/network
  (`cd poc && python3 -m pytest tests`). Separate from the real dev env and
  entirely optional; documented in `poc/README.md`.
